  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* 標籤分布 */}
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">標籤分布 (前10名)</h3>
          <div className="space-y-2">
            {sortedTags.map(([tagName, data]) => (
              <div key={tagName} className="flex items-center justify-between">
                ...
                <span className="text-sm text-gray-600">{data.count} 篇</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* 期刊會議類型分布 */}
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">發表類型分布</h3>
          {(() => {
            const journalCount = papers.filter(p => p.venue?.type === 'journal').length
            const conferenceCount = papers.filter(p => p.venue?.type === 'conference').length
            const noVenueCount = papers.filter(p => !p.venue).length
            const total = journalCount + conferenceCount + noVenueCount
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <span className="text-sm font-medium text-gray-700">期刊論文</span>
                  </div>
                  <div className="text-right">
                    <span className="text-sm font-semibold text-gray-600">{journalCount} 篇</span>
                    <div className="text-xs text-gray-500">
                      {total > 0 ? ((journalCount / total) * 100).toFixed(1) : 0}%
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <span className="text-sm font-medium text-gray-700">會議論文</span>
                  </div>
                  <div className="text-right">
                    <span className="text-sm font-semibold text-gray-600">{conferenceCount} 篇</span>
                    <div className="text-xs text-gray-500">
                      {total > 0 ? ((conferenceCount / total) * 100).toFixed(1) : 0}%
                    </div>
                  </div>
                </div>
                ...
                {noVenueCount > 0 && (
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      ...
                      <span className="text-sm font-medium text-gray-700">未分類</span>
                    </div>
                    <div className="text-right">
                      <span className="text-sm font-semibold text-gray-600">{noVenueCount} 篇</span>
                      <div className="text-xs text-gray-500">
                        {total > 0 ? ((noVenueCount / total) * 100).toFixed(1) : 0}%
                      </div>
                    </div>
                  </div>
                )}
